...
securities = ['JPM']
# factors = ['XLF', 'XLE', 'SMH', 'XLI', 'XLK', 'XLP','IYT','OIH', 'XLV', 'XLY', 'RKH']
...
def get_Predict_Resi(data, Liner_Object):
    ...
    resid = Y - Liner_Object.predict(sm.add_constant(indep, prepend=True))
    return resid
...
#                 self.order(factors[0], int(2000*regr_params[0]))
                self.order_exec(2000, -1, regr_params)
                self.invested = True 
...
if __name__ == '__main__':
    ...
    result.to_csv("zscores_orders_position_2" + __name__ + ".csv")
